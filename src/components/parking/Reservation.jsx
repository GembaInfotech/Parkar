...
const Reservation = ({ data, inT, ouT }) => {
  ...
  useEffect(() => {
    ...
    const newAmt = data?.priceF * (quotient + val);
    const newAmtT = data?.priceT * (quotient + val);
    setAmt(newAmt);
    setAmtT(newAmtT);
  ...
  const launch = () => {
    ...
    navigate(`/checkout?name=${data.name}&id=${data._id}&c=${data.code}&in=${inT}&out=${ouT}&amt=${amt}&amtT=${amtT}&diff=${difference}`)

  }
  return (
    <div className=' sticky sm:ml-4 shadow-2xl dark:bg-white rounded-2xl my-2 sm:mt-16  sm:w-1/3  h-full sm:h-96'>
      ...
      <div className='px-4 sm:px-8 '>
        <div className=' border border-1 border-gray-400  rounded-3xl p-4 '>
          <div className='flex  justify-between '>
            <div>
              <p>Basic Price for Four Wheeler</p>
            </div>
            <div className=' '>
              <div className='flex'>
          ...
          <div className='flex  justify-between '>
            <div>
              <p>Basic Price for Two Wheeler</p>
            </div>
            <div className=' '>
              <div className='flex'>
                <PiCurrencyInrLight className='pt-1 text-orange-400 font-bold text-3xl ' />
                <p className='text-orange-400 font-bold text-xl sm:text-2xl '> {amtT} </p>
              </div>
              <p className='text-[12px] text-gray-500'>for {Math.round(difference / 60)} hours</p>
            </div>
          </div>
